...
* skillBaseId: {
    ...
    * chargeStages: **final stage** - Value of the stage of the final charging of the skill (if the skill is chargeable)
...
* skillInterval: **interval** - Sets the frequency of repeated cast of the skill within the specified **interval** ms
...
* inHold: **true | false** - If set to true, the action is executed even if the time specified in **holdDuration** has not elapsed
...
* enableIfEdge: **edge | [edges]** - This action is only executed if all of these Sorcerer edges (*fire*, *ice*, *lightning*) are active
* disableIfEdge: **edge | [edges]** - This action is only executed if all of these Sorcerer edges (*fire*, *ice*, *lightning*) are not active
